...
// === PANGGIL AI (DENGAN MODE THINKING & MEMORY) ===
async function callAI(messagesHistory, isThinking) {
    if (!config.baseUrl || !config.model) return '⚠️ Bot belum disetup.';
    try {
        const url = config.baseUrl.replace(/\/+$/, '') + '/chat/completions';
        
        // Beda System Prompt buat Normal vs Thinking
        ...
        const res = await axios.post(url, {
            ...
            timeout: 180000 // Timeout 3 menit khusus buat mode thinking yang mikir lama
        });
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if (connection === 'close') {
            ...
            if (reason !== DisconnectReason.loggedOut) {
                ...
                console.log('❌ Logout. Hapus folder auth (rm -rf auth) kalo mau login lagi.');
            }
        } else if (connection === 'open') {
            console.log('✅ Bot online dan siap dipakai! Kirim /ai di WA lu.\n');
        }
    });
        ...
        try {
            ...
            if (body.toLowerCase().startsWith('set')) {
                ...
                if (updated) {
                    ...
                    return sock.sendMessage(jid, { text: "❌ Format salah. Pastikan ada kata 'endpoint:', 'apikey:', dan 'model:'." }, { quoted: msg });
                }
            }

            // 🧹 2. COMMAND /ai clear (HAPUS INGATAN)
            ...
            // 🖼️ 5. DOWNLOAD GAMBAR (KALO ADA)
            if (hasImage) {
                try { imageBuffer = await downloadMediaMessage(msg, 'buffer', {}, { logger: pino({level:'silent'}) }); } 
                catch (e) { await sock.sendMessage(jid, { text: '❌ Gagal mengunduh gambar.' }, { quoted: msg }); return; }
            }

            if (!config.baseUrl || !config.model) { 
                return sock.sendMessage(jid, { text: '⚠️ Bot belum disetup. Ketik /ai untuk panduan setup.' }, { quoted: msg }); 
            }

            // 📚 6. MASUKIN KE INGATAN (HISTORY)
            if (!chatHistory[jid]) chatHistory[jid] = [];
            chatHistory[jid].push({ role: 'user', content: prompt }); // Simpan teks aja biar gak berat
            if (chatHistory[jid].length > 10) chatHistory[jid] = chatHistory[jid].slice(-10); // Maks 10 pesan terakhir

            // Siapkan payload buat API (Suntik gambar ke pesan terakhir kalo ada)
            ...
            console.log(`[AI] User ${sender} nanya (${isThinking ? 'THINKING' : 'NORMAL'}), AI lagi ngetik live...`);
            ...
            try {
                ...
                clearInterval(typingInterval);
                await sock.sendPresenceUpdate('paused', jid);
                await sock.sendMessage(jid, { text: '❌ Terjadi error pas AI lagi mikir.' }, { quoted: msg });
            }
...
// === LOGIKA START & PAIRING CODE (ANTI NGACO) ===
async function startSock() {
    ...
    if (!state.creds.registered) {
        ...
        await ask('Siapkan HP lu. Buka WA -> Tautkan perangkat -> Tautkan dengan nomor. Tekan ENTER...');
        ...
        try {
            console.log('🔄 Sabar, lagi minta kode ke server WA... (Tunggu 3 detik)');
            ...
            console.log(`====================================\n`);
            console.log('Masukin kodenya di HP lu sekarang. Gua tungguin sampe sukses...\n');
            ...
            console.error('❌ Gagal minta kode. Error:', e.message);
            console.log('Hapus folder auth (rm -rf auth) terus jalanin lagi bang.');
            process.exit(1);
        }
    }

    console.log('\n✅ Udah login. Lanjut ke proses bot...\n');
    setupBotListeners(sock, saveCreds);
}
